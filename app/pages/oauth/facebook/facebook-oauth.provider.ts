...
import { Facebook, CordovaOauth } from 'ng2-cordova-oauth/core';
...
export class FacebookOauthProvider implements IOathProvider {
	...
	constructor(http: Http, config: Config) {
		this.http = http;
		this.config = config;
		let facebook = new Facebook({ clientId: config.facebook.appId, appScope: config.facebook.scope });
		this.cordovaOauth = new CordovaOauth(facebook);
	}
